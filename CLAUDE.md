...
| **マネージャー（作業A担当兼任）** | エージェント（Claude Code） | ・全体の調整・管理<br>・作業A（プレビューUI）の実装 |
...
**GitHub操作（git push）はマネージャー（作業A担当）だけが行います。**

| 操作 | 誰が行うか |
|------|-----------|
| `git add` / `git commit` | 各作業担当エージェント |
| `git push` | **マネージャー（作業A担当）だけ** |
| `git pull` | マネージャーだけ |

**作業B・C・D担当は、絶対に `git push` を実行しないでください。**

作業完了時のフロー:
1. 各作業担当: `git add` と `git commit` を実行（ローカルコミットまで）
2. マネージャーに進捗報告
3. マネージャーが全変更を確認し、`git push` を実行
